        ...
        <v-card>
          <v-card-title primary-title >
            <div>
                ...
                <span class="grey--text">{{ pelicula.anhopub }}
                    &middot; {{ pelicula.genero }}</span>
                </div>
          </v-card-title>
          <h6 class="card-title" v-if = "current_user"
            @click="calificar()">Calificar esta pelicula </h6>
          <!-- <h6 class="card-title">Calificar esta pelicula </h6> -->
          <v-card-text>
            {{ pelicula.sinopsis }}
          </v-card-text>
        </v-card>
...
const ComponenteCalif = Vue.extend({
  ...
  template: `
      <div class= "rating">
          Cual es la calificacion de esta pelicula ?
          <star-rating- v-model="calif" :show-rating="false">
          </star-rating>
      </div>
  `,
...
export default{
  ...
  methods: {
    async calificar() {
      this.$swal({
        ...
        return axios({
          ...
          url: `https://localhost:8081/peliculas/calif/${peliculaId}`,
          headers: {
            'Content-Type': 'application/json',
          },
        }).then(() => {
          this.$swal(
            `Gracias por calificar! ${estado.nota}`,
            'success',
          );
        }).catch((error) => {
          const mensaje = error.respuesta.data.message;
          this.$swal('Oh no puede ser !',
            `${mensaje}`, 'error');
        });
      });
    },
    async obtenerPelicula() {
      return axios({
        method: 'get',
        url: `https://localhost:8081/api/peliculas/${this.$route.params.id}`,
      })
        .then((respuesta) => {
